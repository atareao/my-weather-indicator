...
URL_FIND_CITY = 'http://api.openweathermap.org/data/2.5/find?lat=%s&lon=%s&cnt=1'
'''
{"message":"Model=GFS-OWM, ","cod":"200","calctime":0.0205,"cnt":1,"list":[{"id":2519752,"dt":1347083047,"name":"Castello de la Plana","coord":{"lat":39.98,"lon":-0.03},"main":{"temp":293.51,"humidity":48,"pressure":1013,"temp_min":293,"temp_max":294},"wind":{"speed":0,"deg":3},"clouds":{"all":15},"rain":{"3h":0}}]}
'''
URL_CURRENT_CITY_ID = 'http://api.openweathermap.org/data/2.5/weather?id=%s&appid='+APPID
URL_FORECAST_CITY_ID = 'http://api.openweathermap.org/data/2.5/forecast/daily?id=%s&cnt=7&mode=json&appid='+APPID
URL_HOURLY_CITY_ID = 'http://api.openweathermap.org/data/2.5/forecast?id=%s&appid='+APPID

URL_CURRENT_CITY_LL = 'http://api.openweathermap.org/data/2.5/weather?lat=%s&lon=%s&appid='+APPID
URL_FORECAST_CITY_LL = 'http://api.openweathermap.org/data/2.5/forecast/daily?lat=%s&lon=%s&appid='+APPID
URL_HOURLY_CITY_LL = 'http://api.openweathermap.org/data/2.5/forecast?lat=%s&lon=%s&appid='+APPID
...
if __name__ == "__main__":
	...
	#latitude = 42.98
	#longitude = -81.23
	#print(find_city(longitude,latitude))
	...
	for data in owm.get_hourly_weather():
		...
		result += str(data['datetime']) +','+str(data['temperature'])+','+str(data['avehumidity'])+','+str(data['cloudiness'])+'\\n'
	print(result)
