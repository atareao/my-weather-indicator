...
CONDITIONS = {}
CONDITIONS['blowing sand']={'text':_('Blowing sand'),'image':'mwig-windy.png','image-night':'mwig-windy.png','icon-dark':'mwid-windy.png','icon-night-dark':'mwid-windy.png','icon-light':'mwil-windy.png','icon-night-light':'mwil-windy.png'}
CONDITIONS['blizzard']={'text':_('Blizzard'),'image':'mwig-windy.png','image-night':'mwig-windy.png','icon-dark':'mwid-windy.png','icon-night-dark':'mwid-windy.png','icon-light':'mwil-windy.png','icon-night-light':'mwil-windy.png'}
CONDITIONS['blowing snow']={'text':_('Blowing snow'),'image':'mwig-snow.png','image-night':'mwig-snow.png','icon-dark':'mwid-snow.png','icon-night-dark':'mwid-snow.png','icon-light':'mwil-snow.png','icon-night-light':'mwil-snow.png'}
CONDITIONS['blowing widespread dust']={'text':_('Blowing widespread dust'),'image':'mwig-windy.png','image-night':'mwig-windy.png','icon-dark':'mwid-windy.png','icon-night-dark':'mwid-windy.png','icon-light':'mwil-windy.png','icon-night-light':'mwil-windy.png'}
CONDITIONS['chance of a thunderstorm']={'text':_('Chance of a thunderstorm'),'image':'mwig-chance-of-thunderstorms.png','image-night':'mwig-chance-of-thunderstorms-night.png','icon-dark':'mwid-chance-of-thunderstorms.png','icon-night-dark':'mwid-chance-of-thunderstorms-night.png','icon-light':'mwil-chance-of-thunderstorms.png','icon-night-light':'mwil-chance-of-thunderstorms-night.png'}
CONDITIONS['chance of flurries']={'text':_('Chance of flurries'),'image':'mwig-chance-of-snow.png','image-night':'mwig-chance-of-snow-night.png','icon-dark':'mwid-chance-of-snow.png','icon-night-dark':'mwid-chance-of-snow-night.png','icon-light':'mwil-chance-of-snow.png','icon-night-light':'mwil-chance-of-snow-night.png'}
CONDITIONS['chance of rain']={'text':_('Chance of rain'),'image':'mwig-chance-of-rain.png','image-night':'mwig-chance-of-rain-night.png','icon-dark':'mwid-chance-of-rain.png','icon-night-dark':'mwid-chance-of-rain-night.png','icon-light':'mwil-chance-of-rain.png','icon-night-light':'mwil-chance-of-rain-night.png'}
CONDITIONS['chance of showers']={'text':_('Chance of showers'),'image':'mwig-chance-of-rain.png','image-night':'mwig-chance-of-rain-night.png','icon-dark':'mwid-chance-of-rain.png','icon-night-dark':'mwid-chance-of-rain-night.png','icon-light':'mwil-chance-of-rain.png','icon-night-light':'mwil-chance-of-rain-night.png'}
...
def degToCompass(num):
    ...
    arr=['n','nbe','nne','nebn','ne','nebe','ene','ebn','e','ebs','ese',
...
def get_dayLength(day,longitude,latitude):
	sun = Sun()
	return sun.dayLength(day.year, day.month, day.day,  s2f(longitude), s2f(latitude))

def get_dayCivilTwilightLength(day,longitude,latitude):
	sun = Sun()
	return sun.dayCivilTwilightLength(day.year, day.month, day.day,  s2f(longitude), s2f(latitude))

def get_dawn(day,longitude,latitude,rawOffset):
	sun = Sun()
	ss = sun.civilTwilightLocal(day.year, day.month,day.day, s2f(longitude),s2f(latitude),rawOffset)
	return '%s'%(ss[0])
...
def get_sunrise(day,longitude,latitude,rawOffset):
	sun = Sun()
	ss = sun.sunRiseSetLocal(day.year, day.month,day.day, s2f(longitude),s2f(latitude),rawOffset)
	return '%s'%(ss[0])
...
def get_wind_chill(temperature,wind_velocity):
	...
	if temperature<=50.0 and wind_velocity>=3.0:
		wc = 35.74 + 0.6215*temperature-35.76*math.pow(wind_velocity,0.16)+0.4275*temperature*math.pow(wind_velocity,0.16)
...
def get_heat_index(temperature,humidity):
	...
	if humidity>0.0 and temperature>=77.0:
		...
		+ c5*math.pow(temperature,2.0) + c6*math.pow(humidity,2.0)\
		+ c7*math.pow(temperature,2.0)*humidity + c8*temperature*math.pow(humidity,2.0)\
		+ c9*math.pow(temperature,2.0)*math.pow(humidity,2.0)
...
def change_pressure(valor,a):
	...
	units_m = {'mb':_('millibar'),'in':_('inches of mercury'),'mm':_('millimeters of mercury')}
	if a in units_u.keys():
		if a == 'in':
			digits = 1
		else:
			digits = 0
		return '%s %s'%(locale.str(redondea(valor*units_u[a],digits)),units_m[a])
...
def change_velocity(valor,a):
	valor = s2f(valor)
	# initial a in mph
	units_u = {'mph':1,'km/h':1.609344,'m/s':0.44704,'knots':0.868976,'ft/s':1.466667}
	units_m = {'mph':_('mph'),'km/h':_('km/h'),'m/s':_('m/s'),'knots':_('knots'),'ft/s':_('ft/s')}
	if a in units_u.keys():
		return '%s %s'%(locale.str(redondea(valor*units_u[a])),units_m[a])
	if a == 'Beaufort':
		if valor <= 1:
...
class Units():
	def __init__(self,temperature= 'C',wind = 'km/h',pressure = 'mb', visibility = 'km', snow = 'cm', rain = 'mm',ampm = False):
...
class WeatherService():
	def __init__(self,longitude=-0.418,latitude=39.360, units = Units(), key = ''):
		if not longitude:
			longitude = 0
		if not latitude:
			latitude = 0
	...
	def get_default_values(self):
		current_conditions ={}
		current_conditions['rawOffset'] = geocodeapi.get_rawOffset(self.timezoneId)
		...
		#		
		dayLength = get_dayLength(datetime.today(),self.longitude,self.latitude)
		dayCivilTwilightLength = get_dayCivilTwilightLength(datetime.today(),self.longitude,self.latitude)
		if dayLength == 0.0:
			...
			current_conditions['sunrise_time'] = get_sunrise(datetime.today(),self.longitude,self.latitude,current_conditions['rawOffset'])
			current_conditions['sunset_time'] = get_sunset(datetime.today(),self.longitude,self.latitude,current_conditions['rawOffset'])
			current_conditions['isday'] = is_day_now(current_conditions['sunrise_time'],current_conditions['sunset_time'],current_conditions['rawOffset'])
			current_conditions['sunrise'] = timeformat(current_conditions['sunrise_time'],self.units.ampm)
			current_conditions['sunset'] = timeformat(current_conditions['sunset_time'],self.units.ampm)
		current_conditions['sunrise_time_utc'] = get_sunrise(datetime.today(),self.longitude,self.latitude,0)
		current_conditions['sunset_time_utc'] = get_sunset(datetime.today(),self.longitude,self.latitude,0)
		...
		if dayCivilTwilightLength == 0.0:
			...
			current_conditions['dawn_time'] = get_dawn(datetime.today(),self.longitude,self.latitude,current_conditions['rawOffset'])
			current_conditions['dusk_time'] = get_dusk(datetime.today(),self.longitude,self.latitude,current_conditions['rawOffset'])
			current_conditions['dawn'] = timeformat(current_conditions['dawn_time'],self.units.ampm)
			current_conditions['dusk'] = timeformat(current_conditions['dusk_time'],self.units.ampm)
		...
		dayLength = get_dayLength(datetime.today(),self.longitude,self.latitude)
		dayCivilTwilightLength = get_dayCivilTwilightLength(datetime.today(),self.longitude,self.latitude)
		if current_conditions['sunrise_time'] == current_conditions['sunset_time']:
			if dayLength>0:
				current_conditions['sunrise_time'] = _('Up all day')
				current_conditions['sunset_time'] = _('Up all day')
			else:
		...
		for i in range(0,7):
			...
			if dayLength == 0.0:
				...
				fc['sunrise'] = timeformat(get_sunrise(dia,self.longitude,self.latitude,current_conditions['rawOffset']),self.units.ampm)
				fc['sunset'] = timeformat(get_sunset(dia,self.longitude,self.latitude,current_conditions['rawOffset']),self.units.ampm)
