...
class Sun:
    ...
    def civilTwilightLocal(self, year, month, day, lon, lat, rawOffset):
        """
        This macro computes the start and end times of civil twilight.
        Civil twilight starts/ends when the Sun's center is 6 degrees below
        the horizon.
        """
        sr, st = self.__sunriset__(year, month, day, lon, lat, -6.0, 0)
        sr = from_utc_to_local(sr, rawOffset)
        st = from_utc_to_local(st, rawOffset)
        return sr, st
    ...
    def get_max_solar_flux(self, latitude, year, month, day):
        ...
        Information was of the public domain before release by Environment
        Canada Output is in W/M^2.
        """
    ...
    def __Solcons(self, dAlf):
        """
        Name: __Solcons
        Parameters: [I] double dAlf : Solar constant to correct the
        excentricity
...
if __name__ == "__main__":
    ...
    k = Sun()
    # latitud +90 a -90 (Polo Norte - Ecuador - Polo Sur)
    lat = 0.22
    # longitud -180 a 180 (0 es Greenwich)
    lon = 39.28

    am, at = k.civilTwilightLocal(2012, 18, 11, lon, lat, 0)
    print('###########################################################')
    print('Longitud %s' % lon)
    print('Latitud %s' % lat)
    print(am)
    print(at)
    am, at = k.sunRiseSetLocal(2012, 18, 11, lon, lat, 0)
    print('###########################################################')
    print('Longitud %s' % lon)
    print('Latitud %s' % lat)
    print(am)
