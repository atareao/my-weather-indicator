...
def urlEncodeNonAscii(b):
	...
	return b.encode(output_encoding or 'utf-8')
'''

URLDIR_YAHOO = 'http://query.yahooapis.com/v1/public/yql?q=select * from geo.placefinder where text="%s" and gflags="R" and locale="%s"&format=json'
URLINV_YAHOO = 'http://query.yahooapis.com/v1/public/yql?q=select * from geo.placefinder where text="%s,%s" and gflags="R" and locale="%s"&format=json'
URLINV_YAHOO2 = 'http://query.yahooapis.com/v1/public/yql?q=select * from geo.placefinder where text="%s,%s" and gflags="R" and locale="%s"&format=json&appid=dj0yJmk9TTg1VUQwWHh1c0Q0JmQ9WVdrOVlVcFBWSEZrTkdrbWNHbzlNakEyTkRVME1qUTJNZy0tJnM9Y29uc3VtZXJzZWNyZXQmeD02Mg--'
URLINV_YAHOO2 = 'http://gws2.maps.yahoo.com/findlocation?pf=1&locale=%s&offset=15&flags=&q=%s,%s&gflags=R&start=0&count=10&format=json'#locale,lat,lon
def s2f(cadena):
	try:
		value = float(cadena)
	except:
...
def get_timezoneId(lat,lon):
	...
	try:
		response = read_from_url('http://api.geonames.org/timezoneJSON?lat=%s&lng=%s&username=atareao'%(lat,lon)).decode()
...
'''
def get_rawOffset(lat,lon):	
	try:
		print('Requesting timezone')
		response = read_from_url('http://api.geonames.org/timezoneJSON?lat=%s&lng=%s&username=atareao'%(lat,lon)).decode()
		json_response = json.loads(response)		
		if json_response and 'timezoneId' in json_response.keys():
			timezone = pytz.timezone(json_response['timezoneId']);
			hh,mm = (str(timezone.localize(datetime.datetime.now())).split('+'))[1].split(':')
			hh = s2f(hh) + s2f(mm)/60.0#.strftime("%z");
			return hh
		return 0.0
	except:
		print('Geonames error')
		url = 'http://www.earthtools.org/timezone/%s/%s'%(lat,lon)
		response = read_from_url(url).decode()
		inicio = response.find('<offset>')+8
		fin = response.find('</offset>')
		ro = s2f(response[inicio:fin])
		inicio = response.find('<dst>')+5
		fin = response.find('</dst>')
		if response[inicio:fin]=='True':
			ro = ro + 1.0
		return ro
'''
...
def get_directions(search_string):
	directions = []
	try:
		url = URLDIR_YAHOO % (search_string,LANG)
		...
		if jsonResponse['query']['count']>1:
			for ans in jsonResponse['query']['results']['Result']:
				directions.append(fromjson2direction(ans))
		else:
			ans = jsonResponse['query']['results']['Result']
			directions.append(fromjson2direction(ans))
	except Exception as e:
		print('Error:',e)
	return directions
